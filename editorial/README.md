...
| [`profiles/artifact/audio-*.json`](profiles/artifact/) | Three machine-readable audio artifact profiles (monologue, dialogue, timed-narration) — thought-unit, rhythm, recurrence, scaffolding, pace, pronunciation, and renderer-preference rules from `AUDIO-SCRIPT.md`, plus audience adaptation and (for dialogue) speaker-role/persona-boundary state, encoded uniformly with the visual profiles. Planning/script path only — TTS rendering is not yet exercised end to end (deferred from the SUE-570 pilot). | AES-V2.8 (SUE-566) |
| [`../schemas/audio-plan.schema.json`](../schemas/audio-plan.schema.json) + [`../schemas/AUDIO-PLAN-CONTRACT.md`](../schemas/AUDIO-PLAN-CONTRACT.md) | The pre-TTS audio plan contract: canonical spoken script vs delivery state vs render lineage, the script-L1-before-render gate, dialogue knowledge-asymmetry/persona-boundary checks, and rendered-audio QA kept separate from script QA | AES-V2.8 (SUE-566) |
